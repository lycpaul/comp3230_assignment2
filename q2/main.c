...
int main(int argc, char** argv)
{
	...
	for(i = 0; i < num_cars; i++) {
		if (workgroupID < i) {
			//	Ensure this work group have finish previous task
			for (int j=0; j<8; j++) {
				pthread_join(threads[workgroupID*8+j], NULL);
			}
		}
		//	Assign the the task to one work group
		for(int j=0; j<8; j++) {
			int tid = workgroupID*8+j;
		...
		//	switch to the next group, group with less than 7 workers will be ignored
		workgroupID = (workgroupID+1) % (num_workers/8);
	}
	
	for (i=0; i<(num_workers/8)*8; i++) {
